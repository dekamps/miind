...
def generate_fill_in_rate_function():
     ...
     + '\tstd::vector<MPILib::Rate>& vec_activity_rates,\n' \
     + '\tMPILib::Time t\n' \
     + ')\n' \
     + '{\n'\
     ...
     + '\tconst std::vector<MPILib::Rate>& magrates = group.F();\n' \
...
def generate_apply_network_function(nodes,algorithms,connections):
     ...
     s += '\tstd::vector<MPILib::Rate>& vec_mag_rates\n'
     s += '){\n'
...
def generate_log_function():
     ...
     s += 'void LogData(std::ostream& s, MPILib::Time t, const std::vector<MPILib::Rate>& vec_rates)\n'
     s += '{\n'
...
def generate_preamble(fn, variables, nodes, algorithms, connections, cuda):
    ...
    fill_in_function = generate_fill_in_rate_function()
    apply_network_function = generate_apply_network_function(nodes, algorithms, connections)
