...
#include <iostream>
#include <limits>
#include "MeshFiniteSizeAlgorithm.h"
#include "Stat.hpp"
...
namespace TwoDLib {
	...
	pugi::xml_node MeshFiniteSizeAlgorithm<Solver>::CreateRootNode(const string& model_name) {
	...
	template <class Solver>
	std::vector<TwoDLib::Mesh> MeshFiniteSizeAlgorithm<Solver>::CreateMeshObject() {
		// mesh
	...
	template <class Solver>
	std::vector<TwoDLib::Redistribution> MeshFiniteSizeAlgorithm<Solver>::Mapping(const string& type)
	{
		Pred pred(type);
	...
	template<>
	MeshFiniteSizeAlgorithm<TwoDLib::MasterFiniteObject>::MeshFiniteSizeAlgorithm
	(
		...
		_sys(_mesh_vec, _vec_vec_rev, _vec_vec_res, _vec_tau_refractive, 1000),
		_n_evolve(0),
		_n_steps(0),
	...
	template<>
	MeshFiniteSizeAlgorithm<TwoDLib::MasterFiniteObject>::MeshFiniteSizeAlgorithm(const MeshFiniteSizeAlgorithm<TwoDLib::MasterFiniteObject>& rhs) :
	...
	template <class Solver>
	std::vector<TwoDLib::TransitionMatrix> MeshFiniteSizeAlgorithm<Solver>::InitializeMatrices(const std::vector<std::string>& mat_names)
	{
	...
	template <class Solver>
	MeshFiniteSizeAlgorithm<Solver>* MeshFiniteSizeAlgorithm<Solver>::clone() const
	{
		return new MeshFiniteSizeAlgorithm<Solver>(*this);
	}

	template <class Solver>
	void MeshFiniteSizeAlgorithm<Solver>::configure(const MPILib::SimulationRunParameter& par_run)
	{
	...
	template <class Solver>
	void MeshFiniteSizeAlgorithm<Solver>::assignNodeId(MPILib::NodeId nid) {
		_node_id = nid;
	}

	template <class Solver>
	MPILib::AlgorithmGrid MeshFiniteSizeAlgorithm<Solver>::getGrid(MPILib::NodeId id, bool b_state) const
	{
	...
	template <class Solver>
	void MeshFiniteSizeAlgorithm<Solver>::reportDensity(MPILib::Time t) const
	{
		std::ostringstream ost;
		ost << _node_id << "_" << t;
		ost << "_" << _sys.P();
	...
	template <class Solver>
	void MeshFiniteSizeAlgorithm<Solver>::evolveNodeState
	(
	...
	template <class Solver>
	void MeshFiniteSizeAlgorithm<Solver>::FillMap(const std::vector<MPILib::CustomConnectionParameters>& vec_weights)
	{
	...
	template <class Solver>
	void MeshFiniteSizeAlgorithm<Solver>::prepareEvolve
	(
		const std::vector<MPILib::Rate>& nodeVector,
		const std::vector<MPILib::CustomConnectionParameters>& weightVector,
