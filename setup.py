...
def main():
    ...
    rearrange_cmake_output_data = {
        ...
        "miind.build": [],
        "miind.build.apps.MatrixGenerator": (["bin\/gsl.dll"] if platform == "win32" else []) + (["bin\/MatrixGenerator.+", "bin\/Bind.+"] if platform == "win32" else []) + (["share\/miind\/apps\/MatrixGenerator\/.+"] if platform != "win32" else []),
        "miind.build.apps.Projection": (["bin\/gsl.dll"] if platform == "win32" else []) + (["bin\/Projection.+"] if platform == "win32" else []) + (["share\/miind\/apps\/Projection\/.+"] if platform != "win32" else []),
